...
`QuantileDiscretizer` 类来进行分桶操作，也称为离散化。分桶是一种数据预处理技术，用于将连续变量转换成离散变量。在机器学习中，这种技术常用于处理连续属性，使其更适合某些算法或提高模型的泛化能力。
...
object QuantileDiscretizerExample {
  def main(args: Array[String]): Unit = {
    ...
    // 创建示例数据
    val data = Seq((1, 20.0), (2, 40.0), (3, 80.0), (4, 100.0), (5, 300.0))
    val df = data.toDF("id", "features")

    // 配置 QuantileDiscretizer
    val discretizer = new QuantileDiscretizer()
      .setInputCol("features")
      .setOutputCol("featuresBucket")
      .setNumBuckets(3) // 分为3个桶
