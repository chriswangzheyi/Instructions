...
neigh = KNeighborsClassifier(
    n_neighbors=3,  #参与投票的最近数据个数
    weights="distance",  #投票权重：distance表示与距离相关且成反比。距离越近，权重越高
    p = 2 # 计算距离当p=1的时候是欧氏距离
)
```
...
```python
grid.best_estimator_.predict(X_test) # 用best_estimator_可以继续调用方法
```
